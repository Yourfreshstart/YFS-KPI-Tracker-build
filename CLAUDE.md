...
**Auth is two separate PIN gates, not real authentication.** `useIdentity()`/`IdentityGate` (team PIN — Teather/Rachael/Jennifer; 8-hour session in `localStorage`) gates Daily Entry, Weekly Ops, Monthly Summary, Payroll, and Pulse. `useCeoAuth(allowedNames)`/`CeoPinGate` (`sessionStorage`, stores *which* name unlocked it, not just a boolean) gates CEO Dashboard (Teather or Jennifer) and Lists/Admin (Teather only — stricter allowedNames, so Jennifer unlocking CEO Dashboard does not also unlock Lists/Admin). Both call the same Postgres RPC `verify_pin(name, pin)` (pgcrypto-hashed). **Supabase RLS policies are wide open (`using (true)`) on every table** — the PIN gates are a UI-layer convenience, not real access control; the anon key can read/write everything directly via the REST API. Don't treat this app as having real auth.
